...
def get_client() -> OpenAI:
    """Lazily initialize OpenRouter client to allow CLI parsing without API key."""
    global client
    if client is None:
        client = OpenAI(
...
# Note: `build_json_schema_prompt` is provided by `data_create.schemas` and
# includes per-field textual instructions suitable for insertion into system
# prompts. See `data_create/schemas.py` for implementation.
...
def process_caselaw_file(input_filepath: str, output_filepath: str) -> float:
    ...
    def add_usage_call(call_name: str, usage):
        ...
        total_tok = getattr(usage, "total_tokens", prompt_tokens + completion_tokens) or 0
    ...
    --- KẾT LUẬN CÁC BÊN (VIỆN KIỂM SÁT) ---
    {json.dumps(stage2_data.get('Noi_dung_vu_an', {}).get('Ket_luan_cac_ben', ''), ensure_ascii=False)}
    ...
    CRITICAL INSTRUCTION - ONE JSON OBJECT PER DEFENDANT:
    For each defendant, create EXACTLY ONE object in PHAN_QUYET_CUA_TOA_SO_THAM. Do NOT create multiple objects for the same defendant.
    - If a defendant has multiple offenses, put ALL offense names into the SINGLE Pham_Toi list within that one object.
    - Example: If defendant "Nguyễn Văn A" is charged with "theft" AND "fraud", create ONE object with Pham_Toi: ["theft", "fraud"], not two separate objects.
    - Match defendant name (Bi_Cao) exactly from the defendant list and case text.

    CRITICAL INSTRUCTION FOR DE_NGHI_CUA_VIEN_KIEM_SAT:
    Use the "KẾT LUẬN CÁC BÊN (VIỆN KIỂM SÁT)" section as the primary source for prosecutor recommendations.
    - For each defendant with a stated recommendation, create ONE object in De_Nghi_Cua_Vien_Kiem_Sat.
    - Match Bi_Cao exactly to Thong_Tin_Bi_Cao.
    - Put all offense names into the single Pham_Toi list for that defendant.
    - If no prosecutor recommendation is present for any defendant, return an empty list.
    ...
    You MUST respond with ONLY a valid JSON object matching this schema. Do not include any explanation or markdown.
    ...
    try:
        ...
        extracted_data = parse_llm_json(raw_text, LLMExtractionOutput)
        ...
        if stage2_data.get("QUYET_DINH", "").strip() and not stage3_data["PHAN_QUYET_CUA_TOA_SO_THAM"]:
            ...
            CRITICAL: For each defendant, create EXACTLY ONE object. If a defendant has multiple offenses, put all offense names in the single Pham_Toi list, not as separate objects.
            ...
            try:
                ...
                retry_data = parse_llm_json(retry_raw, VerdictOnlyOutput)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir",
        default="data_create/Chuong_XXII_route",
